...
function InstCourseDetails(props) {
    ...
    const ShareUrl = (url) => {
        instructorService.contentAccess(url)
            .then(res => {
                setShareUrlData(DMS_URL + "/" + res.data);
    ...
    const newArr = selectedRows.map(({ contentId, lastModifiedDate, lastUpdatedBy, publishStatus, shareUrl, streamingStatus, uploadDate, userId, ...rest }) => {
        ...
        let newPath = originalUrl.replace(DMS_URL, '');
    ...
    return (
        <div className="main-wrapper course-details-page" onLoad={() => scrollWin()}>
            ...
            <HeaderTwo />
            {/* Breadcroumb */}
            < BreadcrumbBox title={getCourseDetails.courseName} bannerUrl={`${COURSE_URL}/${getCourseDetails.banner}`} />
                        ...
                        <div className="course-details-top">
                            ...
                            <div className='child1'>
                                ...
                                {/* <div className="course-details-banner">
                                    <img src={process.env.PUBLIC_URL + `${COURSE_URL}/${getCourseDetails.imageUrl}`} alt="Course Image" style={{ height: '470px', width: '740px' }} className="img-fluid" />
                                </div> */}
                                <div className="course-details-banner" style={{ height: '470px', width: '740px' }}>
                                    {getCourseDetails.video ? (
                                        <video
                                            src={process.env.PUBLIC_URL + `${COURSE_URL}/${getCourseDetails.video}`}
                                            alt="Course Video"
                                            className="img-fluid"
                                            controls
                                        ...
                                        <img
                                            src={process.env.PUBLIC_URL + `${COURSE_URL}/${getCourseDetails.imageUrl}`}
                                            alt="Course Image"
                                            className="img-fluid"
                                        />
                                    )}
                                        ...
                                        <>
                                            {/* `${process.env.PUBLIC_URL + "/dashboard/courseId/:cId"}` */}
                                            {
                                                isDecodeValid === "VALID"
                                                    ?
                                                    <>
                                                        <a className="btn btn-success btn-lg btn-block" href={`${Assessment_Authoring}/assessment/authoring/dashboard/courseId/${courseId}`} onClick={() => assementLoaderFunction()} disabled={assementLoader.isLoading ? "true" : ""}>
                        ...
                        getContentType === "jpg" || getContentType === "png" || getContentType === "jpeg" ? <img src={DMS_URL + `/${getUrl}`} width="1100" height="800" />
                            : getContentType === "pdf" ? (
                                <div>
                                    {" "}
                                    <ViewPdf
                                        pdfUrl={`${Frontend_UI}/pdfViewer/${getUrl}`}
                                    />
                                </div>
                            )
