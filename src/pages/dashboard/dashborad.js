...
function Dashborad() {
    ...
    const enrolledCourseListColumn = [
        ...
        },
        {
            name: 'Action',
            cell: row => <>
                <Button onClick={() => { onClickMoreDetails(row.id.courseId, row.courseDetails.courseName) }} disabled={moreDetailsLoading.isLoading ? "true" : ""} style={{ background: "green", border: "0px" }}> {moreDetailsLoading.isLoading ? (<>{t('loading')}</>) : "More Details"} </Button>
            </>
        }
