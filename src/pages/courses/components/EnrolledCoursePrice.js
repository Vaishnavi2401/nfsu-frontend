...
const EnrolledCoursePrice = (props) => {
    ...
    function calculatePaidCourses() {
        courseState.map((item) => {
            if (item.courseDetails.course_Fee > 0) {
                freeCoursesLength++;
            }
        });
    ...
    function calculateAllCourses() {
        courseState.map((item) => {
            if (item.courseDetails.course_Fee > 0 || item.courseDetails.course_Fee == 0) {
                allCoursesLength++;
            }
        });
...
export default EnrolledCoursePrice
