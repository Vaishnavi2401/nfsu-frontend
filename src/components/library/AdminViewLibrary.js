...
function AdminViewLibrary(props) {
    ...
    const dCourseEnd = new Date(courseValue.closingDate);
    const dCourseStart = new Date(courseValue.commencementDate);
    const denrollReqDate = new Date();
    ...
    const certificateDownload = () => {
        axios({
            method: 'post',
            url: `http://gfsu3.hyderabad.cdac.in:8085/CourseCatalouge/certificate?tenantId=${tenantId}&courseId=${courseId}&emailId=${userId}&name=${username}`,
            responseType: 'blob'
        })
            .then((res) => {
                const file = new Blob(
    ...
    const courseStructurContentView = (contentType, fileUrl) => {
        if (contentType == "youtube") {
            ...
            instructorService.contentAccess("http://gfsu3.hyderabad.cdac.in:8080/" + fileUrl)
                .then(res => {
                    setUrl(res.data);
                    setContentType(contentType);
                    setUrlModal({ show: true });
                    //window.open("http://gfsu3.hyderabad.cdac.in:8080/" + res.data, "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,width=1000,height=1000");
                }).catch(err => {
                    swal(`${t('error')}`, `${t('try_sometimes')}`, "error");
                })
        }
    ...
    const videoJsOptions = {
        ...
        sources: [
            {
                src: `http://gfsu3.hyderabad.cdac.in:8080/${getUrl}`,
                type: 'video/mp4',
            },
        ]
    };
    ...
    return (
        <div className="main-wrapper course-details-page" onLoad={() => scrollWin()}>
            ...
            {/* Breadcroumb */}
            {/* < BreadcrumbBox title="Course Details" bannerUrl={`http://gfsu3.hyderabad.cdac.in:8082/${courseValue.banner}`} /> */}
            ...
            <Styles>
                            ...
                            <div className="course-details-top">
                                <Col lg="8" md="7" sm="12">
                                    ...
                                    <div className="course-details-banner">
                                        <img src={`http://gfsu3.hyderabad.cdac.in:8082/${courseValue.imageUrl}`} alt="" className="img-fluid" style={{ height: 425, width: 825 }} />
                                    </div>
                                </Col>
                ...
                <Modal
                    ...
                    <Modal.Body>
                        {
                            getContentType === "jpg" || getContentType === "png" || getContentType === "jpeg" ? <img src={`http://gfsu3.hyderabad.cdac.in:8080/${getUrl}`} width="1100" height="800" />
                                : getContentType === "pdf" ? <iframe width="1100" height="800" src={`http://gfsu3.hyderabad.cdac.in:8080/${getUrl}`} type="application/pdf" ></iframe>
                                    : getContentType === "mp4" ? <div> <Videojs {...videoJsOptions} /></div>
                                        : getContentType === "docx" ? <iframe width="100%" height="100%" src={`http://gfsu3.hyderabad.cdac.in:8080/${getUrl}`} ></iframe>
                                            : getContentType === "html" ? <iframe width="1100" height="800" src={`http://gfsu3.hyderabad.cdac.in:8080/${getUrl}`} ></iframe>
                                                : getContentType === "zip" ? <iframe width="1100" height="800" src={`http://gfsu3.hyderabad.cdac.in:8080/${getUrl}`} ></iframe>
                                                    : getContentType === "scorm" ? <iframe width="1100" height="800" src={`http://gfsu3.hyderabad.cdac.in:8080/${getUrl}`} ></iframe>
                ...
                <Modal centered show={getCertificateView.show} onHide={() => CertificateViewHide()} backdrop="static" className='custom-modal-style' size="xl" >
                    ...
                    <Modal.Body>
                        ...
                        <div class="fgh"><embed id="fgh" src="http://gfsu2.hyderabad.cdac.in:8083/certificates/246.pdf" type="application/pdf" width="400" height="400"/></div> */}
                            ...
                            <iframe src={`http://gfsu2.hyderabad.cdac.in:8083/${certificateUrl}`} type="application/pdf" style={{ width: '1115px', height: '800px' }}></iframe>
                        </div>
                    </Modal.Body>
